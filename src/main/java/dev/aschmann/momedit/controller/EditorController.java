...
public class EditorController implements Initializable {
    ...
    public void onAddArtifactClick(ActionEvent event) {
        ...
        buildArtifactDialog(
            new Artifact(
                saveGame.getArtifacts().size()
            )
        );
    }

    public void onEditArtifactClick(Object data) {
        if (data instanceof Artifact) {
            buildArtifactDialog((Artifact)data);
        }
    }
    ...
    private void intArtifactTable(List<Artifact> artifacts, TableView<Artifact> table) {
        ...
        table.getColumns().add(getEditButton());
    ...
    private TableColumn<Artifact, Void> getEditButton() {
        TableColumn<Artifact, Void> editButton = new TableColumn<>("edit");

        Callback<TableColumn<Artifact, Void>, TableCell<Artifact, Void>> cellFactory = new Callback<TableColumn<Artifact, Void>, TableCell<Artifact, Void>>() {
            @Override
            public TableCell<Artifact, Void> call(final TableColumn<Artifact, Void> param) {
    ...
    private void buildArtifactDialog(Artifact artifact) {
        ...
        saveGame.getEnchantments().forEach((eName, eOffset) -> {
            ...
            cb.setId(eOffset);
            cb.setText(eName);
            cb.setSelected(artifact.hasEnchantment(eOffset));
